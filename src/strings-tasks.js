...
function getStringLength(value) {
  ...
  // If the value is not a string, or is null/undefined, return 0
  return 0;
}
...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndex !== -1) {
    // Create a new string by concatenating the parts before and after the last occurrence
    return (
...
function formatTime(minutes, seconds) {
  ...
  // Use padStart to format minutes and seconds with two digits
  const formattedMinutes = String(minutes).padStart(2, '0');
  const formattedSeconds = String(seconds).padStart(2, '0');

  // Concatenate the formatted values to create the time string
...
function countVowels(str) {
  // Use the match method with a case-insensitive regular expression to count vowels
...
function invertCase(str) {
  // Use the map method to invert the case of each character
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    ...
    return String.fromCharCode(
      offset + ((char.charCodeAt(0) - offset + 13) % 26)
    );
  });
}
...
function getCardId(value) {
  ...
  // Calculate the index using the ranks and suits

  const rankIndex = ranks.indexOf(rank);
  const suitIndex = suits.indexOf(suit);

  // Return the calculated index
  return rankIndex + suitIndex * ranks.length;
}
